...
function CitySection({ data }) {
  const cities = [...data.city_compare].sort((a, b) => b.total - a.total);
  const max = cities[0].total;

  return (
    <section className="section" id="cities">
      <span className="section-num">07 / Cities</span>
      <div className="shell">
        <div className="section-head">
          <div>
            <span className="eyebrow">07 — Cities at a glance</span>
            <h2 style={{marginTop: 14}}>India's startup geography.</h2>
          </div>
          <p className="lede">
            Company counts and capital raised across India's top six metros.
            Detailed city dashboards ship through 2026.
          </p>
        </div>
        ...
        <div className="card card-pad">
          <div className="card-head">
            <div>
              <div className="card-title">Indian startup ecosystems</div>
              <div className="card-sub">Bar shows companies relative to the largest market</div>
            </div>
          </div>
          <div className="cities-list">
            {cities.map((c, i) => (
              <div key={c.city} className="city-row">
                <span className="city-rank">0{i+1}</span>
                <span className="city-name">
                  {c.city}
                  <span className="coming-soon-tag">{c.current ? "Live" : "Coming Q2"}</span>
                </span>
                <div className="city-bar">
                  <div className="city-bar-fill" style={{width: ((c.total/max)*100) + "%"}}></div>
                </div>
                <span className="city-num">{c.total.toLocaleString()}</span>
                <span className="city-funding">${c.funding_b}B</span>
              </div>
            ))}
          </div>
        </div>
...
Object.assign(window, { FundingSection, ThemeSection, CitySection });
